...
bool Continua(int N){
	...
	/* Percorre todas as solicitações, mesmo que o disco ja esteja 
	 * cheio, pra não atrapalhar a próxima leitura */
	for(i = 0; i < N; i++){
		scanf("%s", tipo);
		if(strcmp(tipo, "insere") == 0){
			scanf("%s %d%c%c", nome, &tam, &unidade, &lixo);
			/* Só faz as contas se for necessário */
			if(!cheio){
...
/* Varre a lista imprimindo as células quando apropriado */
void imprime_disco(Disco ini, int D){
	double parte = D/8.0;
	...
	for(p = ini->dir; p != ini; p = p->dir){
		...
		if(jl == parte){
			...
			tam = p->tam;
			while(jl > parte){ /*(tag1)*/
				/* Quando o nó não está livre */
				if(p->ocupado){
					/* volta ao jl imediatamente anterior */
					jl -= tam;
					/* Calcula quanto falta pra encher a parte */
					resto = parte - jl;
					/* Completa a parte */
					jl += resto;
					/* Imprime a porcentagem livre, que pode não ser nula */
					imprime_celula(livres/jl);
					/* O novo tamanho é o que sobra do tamnho original */
					tam -= resto;
					/* Já le o novo tamanho */
					jl = tam;
					/* Tudo de livres já foi impresso */
					livres = 0;
				/* Quando o nó está livre */
				}else{
					jl -= tam;
					livres -= tam;	
					resto = parte - jl;
					...
					tam -= resto;
					jl = tam;
					/* O novo livres será o que restou 
					 * de tam já que p é livre */
					livres = tam;
				}
			}
		}
	/* O meu loop às vezes não termina de imprimir tudo e pode 
	 * sobrar um pedaço que é com certeza menor que parte, pois
	 * caso contrário tria entrado no while (tag1)*/
	}if(jl > 0){
		imprime_celula(livres/parte);
	}
...
bool insere_arquivo(Disco disco, int D, int tam, char* nome){
	...
	Lista menor_livre_inicial;
	
	/* Se o disco nem tem espaço a função já devolve disco cheio */
	if(disco->tam < tam) return true;

	menor_livre_inicial = (Lista)malloc(sizeof(No));
	/* Recebe D*2 de tamanho porque a comparação é por iferioridade absoluta
	 * e é bom que se houver um local para inserção, mesmo que de tamnho D,
	 * este deve ser escolhido. */
	menor_livre_inicial->tam = D*2;
	menorlivre = menor_livre_inicial;
	...
	/* Frescura do meu compilador: precisa devolver algo como padrão
	 * a função nunca vai chegar aqui. */
	return false;
}
...
bool otimiza_disco(Disco disco, int D){
	...
	/* O tamanho livre total continua armazenado no nó cabeça conforme
	 * explicado nas funções de atribuição e remoção do TAD Lista */
	cria_ultimo(disco, "", disco->tam);
	
	return false;
}
/* Libera arquivos ocupados com o nome  dado e mescla com adjacentes livres */
void remove_arquivo(Disco disco, char* nome){
	Arq* p;
	for(p = disco->dir; p != disco; p = p->dir){
...
/* Cria o nó cabeça, que guarda em tam o tamnho livre do disco */
void cria_lista(Head nova_lista, int D){
...
/* Cria um nó livre e o insere no final da lista, antes do no cabeça */
void cria_ultimo(Lista ini, char nome[], int tam){
