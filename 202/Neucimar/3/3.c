...
typedef struct fila_ordenada{ 
	/* Útil pra imprimir a lista durante o processo de debugar */
	int tamanho;
	/*Contador útil pra a testar se a fila ordenada esta vazia*/
	int num_filas;
	Fila *fila;
...
int main() { 
	...
	while(alturas->num_filas > 0 && k < MAX){
		...
		/* Quando achar uma fila não vazia... */
			...
			/* O contador retorna, porque o adjacente 
			 * pode ter altura menor que a atual, se não 
			 * voltasse, este numca seria rotulado */
			k = 0;
		}
	}
